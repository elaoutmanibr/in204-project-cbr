...
bool ImageCV::autoAdjustImageText(cv::Mat &input_image, cv::Mat &output_image)
{
    ...
    cv::cvtColor(input_image, temp_image, cv::COLOR_RGB2GRAY);
    ...
    cv::adaptiveThreshold(temp_image, output_image, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY, 21, 10);
    ...
    cv::cvtColor(output_image, output_image, cv::COLOR_GRAY2RGB);
